...
template<class T>
concept AccessFlags =
  std::is_same_v<T, ClassAccessFlags> ||
  std::is_same_v<T, InnerClassAccessFlags> ||
  std::is_same_v<T, FieldAccessFlags> ||
  std::is_same_v<T, MethodAccessFlags> ||
  std::is_same_v<T, MethodParameterAccessFlags>;
...
public:
  MethodInfo(MethodAccessFlags accessFlags, types::u2 nameIndex, types::u2 descriptorIndex, std::optional<Code> code, std::vector<types::u2> exceptions)
    :
    mAccessFlags(accessFlags), mNameIndex(nameIndex), mDescriptorIndex(descriptorIndex),
    mCode(std::move(code)), mExceptions(std::move(exceptions))
  {}
...
public:
  ...
  ClassFile(
    types::u2 minorVersion, types::u2 majorVersion, std::unique_ptr<ConstantPool> constantPool,
    ClassAccessFlags accessFlags, types::u2 thisClass, types::u2 superClass,
    std::vector<types::u2> interfaces, std::vector<FieldInfo> fields, std::vector<MethodInfo> methods
  ) :
    mMinorVersion(minorVersion), mMajorVersion(majorVersion), mConstantPool(std::move(constantPool)),
    mAccessFlags(accessFlags), mThisClass(thisClass), mSuperClass(superClass),
    mInterfaces(std::move(interfaces)), mFields(std::move(fields)), mMethods(std::move(methods))
  {}

  // Metadata
  //==--------------------------------------------------------------------==//
  types::u2 minorVersion() const { return mMinorVersion; }
  types::u2 majorVersion() const { return mMajorVersion; }
  ClassAccessFlags accessFlags() const { return mAccessFlags; }
  types::u2 thisClass() const { return mThisClass; }
  types::u2 superClass() const { return mSuperClass; }
